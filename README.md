...
VS Code 1.100.0 以降では、VS Code 自身が持つエンコーディング情報(`TextDocument.encoding` API)を使って EUC-JP を判定します。1.100.0 より前のバージョンではこの API が無いため、ファイルのバイト列を調べて EUC-JP かどうかを判定します

判定方式が異なるだけで、変換の挙動そのものはどちらのバージョンでも同じです
