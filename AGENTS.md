...
新しいファイルをリポジトリルートに追加する場合、`.vscodeignore` に追加が必要かどうか確認してください。`.vscodeignore` はVSIXパッケージ(エンドユーザーに配布される成果物)から除外するファイルの一覧で、対象を明示的に列挙する方式(除外リストに無いファイルは同梱される)です。AGENTS.md/CLAUDE.mdのようなAIエージェント専用のファイルはエンドユーザーには不要なため除外しています(`.claude/**` を除外した #637 と同じ理由)。`npx vsce ls` で実際に同梱されるファイル一覧を確認できます。
