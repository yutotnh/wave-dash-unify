...
既知の制約: サンドボックス環境ではネットワーク制限で`update.code.visualstudio.com`に到達できず、`npm test`(Electron版)がローカル実行できないことがある(#627, #654)。その場合は`tsc` / `eslint` / `webpack` / `prettier`のみローカルで検証し、テスト結果はCI(3 OSマトリクス)で確認する旨をPR本文に書く。
